...
void setupWebServer() {
  ...
  // Start scan endpoint - FIXED: Only sets scanStarted, doesn't start measuring
  server.on("/start_scan", HTTP_POST, []() {
    ...
    if (server.hasArg("plain")) {
      ...
      int duration = requestDoc["duration"] | 30;
      Serial.print("Duration: "); Serial.println(duration);
      
      if (!measuring && !resultsShown) {
      ...
      Serial.println("Error: Invalid request body");
      doc["status"] = "error";
      doc["message"] = "Invalid request body";
    }
...
void loop() {
  ...
  // Debug: Print sensor values to Serial
  ...
  // FIXED: Only reset if we're not waiting for finger detection
  if (irValue < 5000) {
    if (measuring) {
