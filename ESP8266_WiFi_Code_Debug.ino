...
void handleStartScan() {
  ...
  // If still no duration found, try to read from stream
  if (!foundDuration) {
    String body = "";
    while (server.available()) {
      body += char(server.read());
    }
    Serial.print("Stream body: "); Serial.println(body);
    ...
    if (body.length() > 0) {
      ...
      if (!error && requestDoc.containsKey("duration")) {
        duration = requestDoc["duration"];
        foundDuration = true;
        Serial.print("Duration from stream JSON: "); Serial.println(duration);
      }
    }
  }
